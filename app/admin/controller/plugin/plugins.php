...
class Plugins extends Base {
	...
	function delete() {
		PluginsList::deactivate($this->plugin, $this->global['site_id']);

		try {
			//remove plugin public files if any
			Files::rmdir(DIR_PUBLIC . DS . 'plugins' . DS . $this->plugin);
		...
		foreach ($files as $file) {
			if ($file) {
				try {
					...
					$this->pluginSlug = PluginsList :: install($file['tmp_name']);
				} catch (\Exception $e) {
					$error                = $e->getMessage();
