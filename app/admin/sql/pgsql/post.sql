-- Posts
	...
	BEGIN
		...
		WHERE 1 = 1 
			...
			THEN
				AND date_part('month',date_added) = :month
			END @IF					

			-- year
			@IF isset(:year) && !empty(:year)
			THEN
				AND date_part('year',date_added) = :year
			END @IF					
	...
	BEGIN
		...
		SELECT 
			...
			THEN
				,date_part('month',date_added) AS "month" 
			END @IF

			@IF isset(:interval) AND :interval == "day"
			THEN
				,date_part('day',date_added) AS "day" 
			END @IF
		...
		GROUP BY 
			...
			@IF isset(:interval) AND (:interval == "month" || :interval == "day")
			THEN
				,date_part('month',date_added)
			END @IF

			@IF isset(:interval) AND :interval == "day"
			THEN
				,date_part('day',date_added)
			END @IF
