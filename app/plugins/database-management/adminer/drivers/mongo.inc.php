 ...
 $_charset = [];
		...
		class Min_Driver extends Min_SQL {
			...
			function select($table, $select, $where, $group, $order = [], $limit = 1, $page = 0, $print = false) {
				...
				try {
					return new Min_Result($connection->_link->executeQuery("$connection->_db_name.$table", new $class($where, ['projection' => $select, 'limit' => $limit, 'skip' => $skip, 'sort' => $sort])));
				} catch (Exception $e) {
	...
	function driver_config() {
		...
		return [
			'possible_drivers' => ['mongo', 'mongodb'],
			'jush'             => 'mongo',
			'operators'        => $operators,
			'functions'        => [],
			'grouping'         => [],
