...
if (isset($_GET['elastic'])) {
 ...
 $_db;
			 ...
			 * @return mixed
			 */
			function rootQuery($path, $content = [], $method = 'GET') {
			 ...
			 * @return mixed
			 */
			function query($path, $content = [], $method = 'GET') {
 ...
 $_rows;
	...
	function table_status($name = '', $fast = false) {
		global $connection;
		$search = $connection->query('_search', [
			'size'         => 0,
			'aggregations' => [
				'count_by_type' => [
					'terms' => [
		...
		if ($mappings) {
			foreach ($mappings as $name => $field) {
				...
				if ($field['properties']) { // only leaf fields can be edited
					unset($return[$name]['privileges']['insert'], $return[$name]['privileges']['update']);
				}
			}
		}
	...
	*/
	function drop_databases($databases) {
		...
		return $connection->rootQuery(urlencode(implode(',', $databases)), [], 'DELETE');
	}
	...
	*/
	function drop_tables($tables) {
		...
		foreach ($tables as $table) { //! convert to bulk api
			$return = $return && $connection->query(urlencode($table), [], 'DELETE');
		}
	...
	function driver_config() {
		...
		foreach ([
			lang('Numbers') => ['long' => 3, 'integer' => 5, 'short' => 8, 'byte' => 10, 'double' => 20, 'float' => 66, 'half_float' => 12, 'scaled_float' => 21],
			lang('Date and time') => ['date' => 10],
			lang('Strings') => ['string' => 65535, 'text' => 65535],
