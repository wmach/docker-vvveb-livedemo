...
ORDER BY a.attnum'
			...
			$row['privileges']     = ['insert' => 1, 'select' => 1, 'update' => 1];
...
AND typelem = 0"
	...
	function create_sql($table, $auto_increment, $style) {
		...
		foreach ($indexes as $index_name => $index) {
			switch ($index['type']) {
				case 'UNIQUE': $return_parts[] = 'CONSTRAINT ' . idf_escape($index_name) . ' UNIQUE (' . implode(', ', array_map('idf_escape', $index['columns'])) . ')';

break;

				case 'PRIMARY': $return_parts[] = 'CONSTRAINT ' . idf_escape($index_name) . ' PRIMARY KEY (' . implode(', ', array_map('idf_escape', $index['columns'])) . ')';
...
break;
	...
	function driver_config() {
		...
		foreach ([ //! arrays
			...
			lang('Network') => ['cidr' => 43, 'inet' => 43, 'macaddr' => 17, 'txid_snapshot' => 0],
