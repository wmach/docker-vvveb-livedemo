...
<?php echo($engines ? "<select name='Engine'>" . optionlist(['' => '(' . lang('engine') . ')'] + $engines, $row['Engine']) . '</select>' . on_help('getTarget(event).value', 1) . script("qsl('select').onchange = helpClose;") : ''); ?>
...
<p>
<?php echo "<select name='partition_by'>" . optionlist(['' => ''] + $partition_by, $row['partition_by']) . '</select>' . on_help("getTarget(event).value.replace(/./, 'PARTITION BY \$&')", 1) . script("qsl('select').onchange = partitionByChange;"); ?>
