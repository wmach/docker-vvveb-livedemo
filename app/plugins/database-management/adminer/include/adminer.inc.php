...
;
	...
	*/
	function dumpOutput() {
		$return = ['text' => lang('open'), 'file' => lang('save')];
...
<?php
			...
			if (DB == '' || ! $missing) {
				...
				if (support('dump')) {
					echo "<a href='" . h(ME) . 'dump=' . urlencode($_GET['table'] ?? $_GET['select']) . "' id='dump'" . bold(isset($_GET['dump'])) . '>' . lang('Export') . "</a>\n";
				}
			}
