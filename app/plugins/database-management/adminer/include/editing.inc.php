...
* @param int
* @param mixed $result
* @param null|mixed $connection2
* @param mixed $orgtables
* @param mixed $limit
...
if ($type && ! isset($types[$type]) && ! isset($foreign_keys[$type]) && ! in_array($type, $extra_types)) {
	...
	echo optionlist(array_merge($extra_types, $structured_types), $type); ?></select><td><input name="<?php echo h($key); ?>[length]" value="<?php echo h($field['length']); ?>" size="3"<?php echo(! $field['length'] && preg_match('~var(char|binary)$~', $type) ? " class='required'" : ''); //! type="number" with enabled JavaScript ?> aria-labelledby="label-length"><td class="options"><?php
...
<td><input type="radio" name="auto_increment_col" value=""><acronym id="label-ai" title="<?php echo lang('Auto Increment'); ?>">AI</acronym><?php echo doc_link([
...
* @param mixed $drop
* @param mixed $create
* @param mixed $drop_created
* @param mixed $test
* @param mixed $drop_test
* @param mixed $location
* @param mixed $message_drop
* @param mixed $message_alter
* @param mixed $message_create
