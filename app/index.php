...
if (! defined('APP')) {
	if (is_installed()) {
		...
		if (isset($_SERVER['REQUEST_URI']) && strpos($_SERVER['REQUEST_URI'], 'admin') !== false) {
			define('APP', 'admin');
			define('PUBLIC_PATH', (V_SUBDIR_INSTALL ? V_SUBDIR_INSTALL : '') . '/public/');
			define('PUBLIC_THEME_PATH', (V_SUBDIR_INSTALL ? V_SUBDIR_INSTALL : '') . '/public/admin/');
		} else {
			define('APP', 'app');
		}*/
...
if (! defined('PUBLIC_PATH')) {
	define('PUBLIC_PATH', (V_SUBDIR_INSTALL ? V_SUBDIR_INSTALL : '') . '/public/');
	define('PUBLIC_THEME_PATH', (V_SUBDIR_INSTALL ? V_SUBDIR_INSTALL : '') . '/public/');
}
			...
			if ($pageCache->isStale()) {
				...
				if ($i >= $waitSeconds) {
					if (! $pageCache->isGeneratingStuck()) {
						return FrontController::notFound(true, __('Server overload!'), 500);
					}
				}
