...
define('WRITABLE_FOLDERS', ['storage', 'storage/cache', 'storage/model', 'storage/compiled-templates', 'config', 'config/sites.php', 'public/media/', 'public/themes']);
...
class Index extends Base {
	...
	function import($noimport = false) {
		...
		try {
			...
			if ($noimport) {
				Admin::add(['username' => 'admin', 'password' => 'admin', 'email' => 'admin@admin.com', 'role_id' => 1]);
			} else {
	...
	function install() {
		...
		if ($this->request->post) {
			...
			if ($theme) {
				@\Vvveb\set_config('sites.default.theme', $theme);
			}
