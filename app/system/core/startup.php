...
if (APP == 'app') {
	...
	define('DIR_THEME', DIR_ROOT . 'public' . DS . APP . DS . 'themes' . DS);
	define('DIR_PUBLIC', DIR_ROOT . 'public' . DS);
}
...
function autoload($class) {
	...
	if ($isSql) {
		...
		if ($isFromPlugin > 0) {
			...
			$pluginName = str_replace(['/plugins/', $file], '', $sqlFile);
...
//set_error_handler('Vvveb\System\Core\myErrorHandler');
...
function initEcommerce() {
	...
	$tax->setZoneRules(222, 3563, 'store');
