...
function get_setting($namespace, $key = null, $default = null, $site_id = SITE_ID) {
	return System\Setting::getInstance()->get($namespace, $key, $default);
}

function set_setting($namespace, $key = null, $value = null, $site_id = SITE_ID) {
	return System\Setting::getInstance()->set($namespace, $key, $value);
}

function delete_setting($namespace, $key = null, $site_id = SITE_ID) {
	return System\Setting::getInstance()->delete($namespace, $key, $value);
}
...
function themeUrlPath() {
	...
	//$theme = 'landing';
	return PUBLIC_THEME_PATH . 'themes/' . $theme . '/';
}
...
function getThemeFolderList($theme = false) {
	$skipFolders    = ['src', 'backup', 'sections', 'blocks', 'inputs', 'css', 'scss', 'fonts', 'img', 'import', 'node_modules', 'screenshots', 'video', 'js'];
...
function getTemplateList($theme = null, $skip = []) {
	...
	$skipFolders    = array_merge(['src', 'backup', 'sections', 'blocks', 'inputs', 'css', 'scss', 'screenshots', 'locale', 'node_modules'], $skip);
